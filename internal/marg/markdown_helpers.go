...
// the line is a markdown list item (`- `, `* `, `+ `, `- [ ]`, `- [x]`, or
// `<n>. `, with any leading whitespace). For numbered lists the next prefix
// increments. For checkbox items the next prefix is always an unchecked
// `[ ]` so a fresh todo follows a finished one.
...
func isCheckboxAt(line []rune, start int) bool {
	...
	if line[start] != '[' || line[start+2] != ']' || line[start+3] != ' ' {
		return false
	}
	mark := line[start+1]
