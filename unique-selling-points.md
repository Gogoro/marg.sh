...
- **Highlight a passage, press `K`, ask Claude about it — without leaving the doc.** Visual-select any chunk and `K` opens an inline chat overlay over the page. The full document and the selected passage are sent in XML envelopes (`<document>` / `<selection>`), so Claude grounds its answer in your text, not generic advice. Streaming replies, multi-turn, multi-line input (`alt+enter`), `@other-file.md` mentions to attach extra context, `gK` to reopen the last conversation. Esc closes (and cancels the in-flight request). `ctrl+y` yanks the reply to the OS clipboard. Other editors push you to the AI sidebar; marg keeps the AI inside the prose.
- **`ctrl+e` flips the chat into "edit mode" — the next send asks Claude for structured edits, not prose.** The reply is parsed into the same proof pipeline you'd get from `:proof`: inline marks under the changed spans, `gA` to accept, `gX` to reject. The AI's edits land in your document, not in a side panel. As far as I can tell, no other markdown editor wires a free-form chat directly into an inline accept-edits flow.
